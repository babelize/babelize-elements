      ...
      <div className="relative mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-16 grid gap-6 sm:grid-cols-3">
          {communityLinks.map((link, index) => (
            <Reveal key={link.name} delay={0.2 + index * 0.1}>
              <a
                href={link.href}
                target="_blank"
                rel="noopener noreferrer"
                className="group flex h-full flex-col items-center justify-center gap-6 rounded-2xl border border-white/[0.05] bg-white/[0.01] px-6 py-12 text-center transition-all hover:bg-white/[0.03]"
              >
                ...
                <div>
                  ...
                  <p className="mt-2 text-sm leading-relaxed text-white/50 group-hover:text-white/70 transition-colors">
                    {link.description}
                  </p>
                </div>
              </a>
