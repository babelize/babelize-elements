...
const translations: Record<
  string,
  { home: string; docs: string; pricing: string; blog: string; cta: string }
> = {
  ...
  return (
    <div className="space-y-0">
      <div className="flex items-center justify-between rounded-t-2xl border border-b-0 border-black/10 dark:border-white/10 bg-black/[0.02] dark:bg-white/[0.02] px-3 py-2">
        <span className="font-mono text-xs text-black/40 dark:text-white/40">navbar</span>
      ...
      >
        <div className="relative">
          <NavBar
            logo={<span className="text-lg font-bold text-zinc-900 dark:text-zinc-50">MyApp</span>}
            links={links}
            locales={locales}
            value={locale}
